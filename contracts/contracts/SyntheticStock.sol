    ...
    ReentrancyGuard 
{
    ...
    // Fee parameters
    uint256 public mintFee = 25; // 0.25% (basis points)
    ...
    function getCollateralizationRatio(address user) external view returns (uint256 ratio) {
        ...
        uint256 aaplPrice = uint256(price);
        uint256 collateralValue = position.collateralAmount * 10**(AAPL_DECIMALS - USDC_DECIMALS);
        uint256 aaplValue = (position.aaplAmount * aaplPrice * 10**(PRICE_DECIMALS)) / 10**AAPL_DECIMALS;
        
        if (aaplValue == 0) return type(uint256).max;
        
        ratio = (collateralValue * 100e18) / aaplValue;
        return ratio;
    }
