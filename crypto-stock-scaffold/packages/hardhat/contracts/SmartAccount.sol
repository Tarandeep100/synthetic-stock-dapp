...
contract SmartAccount is AccessControl, Pausable, ReentrancyGuard {
    ...
    function redeemAndSwap(
        ...
        // Step 2: For simplicity, assume USDC stays in vault and we swap from there
        // In production, would need proper vault withdrawal and swap flow
        
        // Step 3: AA --> Router (swap USDC to desired token)
        if (tokenOut != address(dexRouter.usdcToken())) {
            // This is simplified - in production would need to withdraw from vault first
            tokenReceived = minTokenOut; // Placeholder
        } else {
